...
public class ModRecipeProvider extends RecipeProvider implements IConditionBuilder {
	...
	protected void buildShapelessRecipes(Consumer<IFinishedRecipe> consumer) {
		ShapelessNBTRecipeBuilder.shapeless(ItemRegistry.PAINT_BUCKET.get(), 1, whiteNBT).makeNoReturnRecipe()
	...
	private void slabBlock(Consumer<IFinishedRecipe> consumer, Block result, Block base) {
		ShapedNBTRecipeBuilder.shaped(result.getBlock(), 6, whiteNBT).define('#', base.getBlock()).pattern("###")
				.unlockedBy("has_rgb_" + base.getRegistryName().getPath(), has(base)).save(consumer);
	}

	private void stairBlock(Consumer<IFinishedRecipe> consumer, Block result, Block base) {
		ShapedNBTRecipeBuilder.shaped(result.getBlock(), 4, whiteNBT).define('#', base.getBlock()).pattern("#  ")
				.pattern("## ").pattern("###").unlockedBy("has_rgb_" + base.getRegistryName().getPath(), has(base))
				.save(consumer);
	}
}
