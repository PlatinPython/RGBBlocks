...
public class ColorSlider extends AbstractSlider {
    ...
    public ColorSlider(int x, int y, int width, int height, ITextComponent displayText, double minValue,
                       double maxValue, double currentValue, SliderType type) {
        ...
        setMessage(new StringTextComponent("").append(displayText)
                                              .append(Integer.toString((int) Math.round(this.value *
                                                                                        (maxValue - minValue) +
                                                                                        minValue))));
    }
    ...
    private void renderHueBackground(MatrixStack matrixStack, ColorSelectScreen screen) {
        Function<Integer, Integer> lerp = (pct) -> (int) Math.floor(MathHelper.lerp(pct / 100f,
                                                                                    this.x + 1,
                                                                                    this.x + this.width - 1));
